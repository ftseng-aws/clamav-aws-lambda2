...
function handler () {
  ...
  ### uncomment below
  echo "[function] Downloading File .... $(aws s3 cp s3://avscan-uploadbucket-delta-$ACCOUNTID/$KEY /mnt/fs1/scan/$KEY)"
  echo "[function] Scanning ... using clamdscan $(clamdscan --log=/tmp/clamdscan-delta-$KEY.log --config-file=/opt/bin/scan.conf /mnt/fs1/scan/$KEY > /tmp/scanresults-delta-$KEY.txt)"
  ...
  echo "[function] Copying scanresults.txt logs into s3 $(aws s3 cp /tmp/scanresults-delta-$KEY.txt s3://avscan-scanlogsbucket-$ACCOUNTID/delta/$KEY.log)"
  echo "[function] Copying clamdscan logs into efs $(aws s3 cp /tmp/clamdscan-delta-$KEY.log s3://avscan-scanlogsbucket-$ACCOUNTID/delta/clamdscan-$KEY.log)"
